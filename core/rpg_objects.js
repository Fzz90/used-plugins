...
//   return false;
// };
const alias = BattleManager.endTurn;
...
Game_Action.prototype.apply = function (target) {
  ...
  if (result.isHit()) {
    if (this.item().damage.type > 0) {
      ...
      var critHeal = Math.ceil(value * 0.3);
      $gameVariables.setValue(103, critHeal);
    }
...
Game_BattlerBase.prototype.isFreezing = function () {
  return this.isAppeared() && this.restriction() == 4;
};
...
Game_BattlerBase.prototype.canUse = function (item) {
  ...
  if (!item) {
    ...
    if (eventIds.hasOwnProperty(elementId)) {
      $gameTemp.reserveCommonEvent(eventIds[elementId]);
    }
...
Game_Battler.prototype.addBuff = function (paramId, turns) {
  if (this.isAlive()) {
    if (this.isActor()) {
      // * RUBY EFFECT
      if (
        this.hasWeapon($dataWeapons[75]) ||
        this.hasWeapon($dataWeapons[87]) ||
        this.hasWeapon($dataWeapons[99]) ||
        this.hasWeapon($dataWeapons[111])
      ) {
        this.overwriteBuffTurns(2, turns + 2);
      }
      // * GARNET EFFECT
      if (
        this.hasWeapon($dataWeapons[77]) ||
        this.hasWeapon($dataWeapons[89]) ||
        this.hasWeapon($dataWeapons[101]) ||
        this.hasWeapon($dataWeapons[113])
      ) {
        this.overwriteBuffTurns(3, turns + 2);
      }
      // * ROSE QUARTZ EFFECT
      if (
        this.hasWeapon($dataWeapons[79]) ||
        this.hasWeapon($dataWeapons[91]) ||
        this.hasWeapon($dataWeapons[103]) ||
        this.hasWeapon($dataWeapons[115])
      ) {
        this.overwriteBuffTurns(4, turns + 2);
      }
      // * AMETHYST EFFECT
      if (
        this.hasWeapon($dataWeapons[81]) ||
        this.hasWeapon($dataWeapons[93]) ||
        this.hasWeapon($dataWeapons[105]) ||
        this.hasWeapon($dataWeapons[117])
      ) {
        this.overwriteBuffTurns(5, turns + 2);
      }
    }
...
Game_CharacterBase.prototype.realMoveSpeed = function () {
  return this._moveSpeed + (this.isDashing() ? 0.75 : 0);
};
...
Game_Interpreter.prototype.command101 = function () {
  if (!$gameMessage.isBusy()) {
    $gameTemp.reserveCommonEvent(86);
