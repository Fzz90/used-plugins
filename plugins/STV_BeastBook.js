...
//----- Window -----
var stv_BeastBook_showWindow = String(
  stv_BeastBook_parameters["Show Window"] || "TRUE",
);
var stv_BeastBook_bgPicture = String(
  stv_BeastBook_parameters["Background Picture"] || "",
);
var stv_BeastBook_showBgBeast = String(
  stv_BeastBook_parameters["Show Background Enemy"] || "TRUE",
);
var stv_BeastBook_showBattleBack = String(
  stv_BeastBook_parameters["Show Battle Back"] || "TRUE",
);
var stv_BeastBook_defaultBattleBack = String(
  stv_BeastBook_parameters["Default Battle Back"] ||
    "img/parallaxes/BlueSky.png",
);
var stv_BeastBook_battleBackIndex = Number(
  stv_BeastBook_parameters["Battle Back Index"] || 0,
);

//--- Animated Side View ---
var stv_BeastBook_asvSpeed = Number(
  stv_BeastBook_parameters["ASV Animation Speed"] || 10,
);
var stv_BeastBook_asvPattern = Number(
  stv_BeastBook_parameters["ASV Animated Pattern"] || 0,
);

//----- Functions -----
var stv_BeastBook_fillBehavior = String(
  stv_BeastBook_parameters["Fill Behavior"] || "2",
);
var stv_BeastBook_showID = String(
  stv_BeastBook_parameters["ID in front of Name"] || "TRUE",
);
var stv_BeastBook_showIDunknown = String(
  stv_BeastBook_parameters["ID in front of Unknown Enemy"] || "TRUE",
);
var stv_BeastBook_animateBeast = String(
  stv_BeastBook_parameters["Animate Enemy"] || "TRUE",
);
var stv_BeastBook_bbOpacity = Number(
  stv_BeastBook_parameters["Backbar Opacity"] || 130,
);
var stv_BeastBook_expIcon = Number(stv_BeastBook_parameters["Exp Icon"] || 189);
var stv_BeastBook_goldIcon = Number(
  stv_BeastBook_parameters["Gold Icon"] || 314,
);
var stv_BeastBook_countDiscovered = Number(
  stv_BeastBook_parameters["Count Discovered Variable"] || 0,
);
var stv_BeastBook_bookFullSwitch = Number(
  stv_BeastBook_parameters["Book Full Switch"] || 0,
);
var stv_BeastBook_maxBeasts = Number(stv_BeastBook_parameters["Max Beasts"]);

//----- Text -----
var stv_BeastBook_unknownData = String(
  stv_BeastBook_parameters["Unknown Info"] || "???",
);
var stv_BeastBook_noData = String(
  stv_BeastBook_parameters["None Data"] || "none",
);
var stv_BeastBook_dropsText = String(
  stv_BeastBook_parameters["Drops Text"] || "Items:",
);
var stv_BeastBook_skillsText = String(
  stv_BeastBook_parameters["Skills Text"] || "Skills:",
);
var stv_BeastBook_weaknessText = String(
  stv_BeastBook_parameters["Weakness Text"] || "Weakness",
);
var stv_BeastBook_killsText = String(
  stv_BeastBook_parameters["Kill Counter Text"] || "Kills:",
);

//----- Colors -----
var stv_BeastBook_unknownColor = Number(
  stv_BeastBook_parameters["Unknown Color"] || 7,
);
var stv_BeastBook_skillsColor = Number(
  stv_BeastBook_parameters["Skills Color"] || 3,
);
var stv_BeastBook_dropsSuccessColor = Number(
  stv_BeastBook_parameters["Drops Success Color"] || 6,
);

//----- Switches -----
var stv_BeastBook_statusSwitch = Number(
  stv_BeastBook_parameters["Info Window Switch"] || 0,
);
var stv_BeastBook_elementsSwitch = Number(
  stv_BeastBook_parameters["Weakness Window Switch"] || 0,
);
var stv_BeastBook_parameterSwitch = Number(
  stv_BeastBook_parameters["Parameter Window Switch"] || 0,
);
var stv_BeastBook_showItemsSwitch = Number(
  stv_BeastBook_parameters["Show Items Switch"] || 0,
);
var stv_BeastBook_showSkillsSwitch = Number(
  stv_BeastBook_parameters["Show Skills Switch"] || 0,
);
var stv_BeastBook_showExpSwitch = Number(
  stv_BeastBook_parameters["Show EXP Switch"] || 0,
);
var stv_BeastBook_showGoldSwitch = Number(
  stv_BeastBook_parameters["Show Gold Switch"] || 0,
);
var stv_BeastBook_showKillCounterSwitch = Number(
  stv_BeastBook_parameters["Show Kill Counter Switch"] || 0,
);

//----- Kill Counter -----
var stv_BeastBook_maxKills = Number(
  stv_BeastBook_parameters["Max Kills Value"] || 25,
);
var stv_BeastBook_killsCountColor1 = Number(
  stv_BeastBook_parameters["Kills Bar Color 1"] || 6,
);
var stv_BeastBook_killsCountColor2 = Number(
  stv_BeastBook_parameters["Kills Bar Color 2"] || 17,
);
var stv_BeastBook_killAchievmentSwitch = Number(
  stv_BeastBook_parameters["Kills Achievment Start Switch"] || 0,
);
...
Scene_BeastBook.prototype.createBackground = function () {
  ...
  if (stv_BeastBook_bgPicture) {
    ...
    this._foregroundSprite.bitmap = ImageManager.loadPicture(
      stv_BeastBook_bgPicture,
    );
...
Scene_BeastBook.prototype.create = function () {
  ...
  if (
    $gameSwitches.value(stv_BeastBook_statusSwitch) ||
    !stv_BeastBook_statusSwitch
  )
    this.createStatusWindow();
  if (
    $gameSwitches.value(stv_BeastBook_elementsSwitch) ||
    !stv_BeastBook_elementsSwitch
  )
    this.createElementsWindow();
  if (
    $gameSwitches.value(stv_BeastBook_parameterSwitch) ||
    !stv_BeastBook_parameterSwitch
  )
...
Scene_BeastBook.prototype.createSelectionWindow = function () {
  ...
  this._selectionWindow.setBeastDataWindows(
    this._infoWindow,
    this._parametersWindow,
    this._elementsWindow,
...
Window_BeastBook_Selection.prototype = Object.create(
...
Window_BeastBook_Selection.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_BeastBook_Selection.prototype.setBeastDataWindows = function (
  window1,
  window2,
  window3,
  window4,
...
Window_BeastBook_Description.prototype.constructor =
  Window_BeastBook_Description;

Window_BeastBook_Description.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_BeastBook_Description.prototype.createDescription = function () {
  ...
  if ($beastBook.isRevealed(this._beast.id)) {
    this.drawTextEx(
      beast.meta.desc1,
      this.contents.width / 2 - this.textWidth(beast.meta.desc1) / 2,
      0,
    );
    this.drawTextEx(
      beast.meta.desc2,
      this.contents.width / 2 - this.textWidth(beast.meta.desc2) / 2,
      y * 1,
    );
    this.drawTextEx(
      beast.meta.desc3,
      this.contents.width / 2 - this.textWidth(beast.meta.desc3) / 2,
      y * 2,
    );
    this.drawTextEx(
      beast.meta.desc4,
      this.contents.width / 2 - this.textWidth(beast.meta.desc4) / 2,
      y * 3,
    );
  } else {
    this.changeTextColor(this.textColor(stv_BeastBook_unknownColor));
    this.drawText(
      stv_BeastBook_unknownData,
      this.contents.width / 2 - this.textWidth(stv_BeastBook_unknownData) / 2,
      0,
    );
    this.changeTextColor(this.normalColor());
  }
...
Window_BeastBook_Info.prototype.contentDrawItems = function () {
  ...
  for (var j = 0; j < 3; j++) {
    ...
    if (di.kind > 0) {
      if (
        $beastBook.beasts[this._beast.id] &&
        $beastBook.beasts[this._beast.id].discoveredItems[j]
      ) {
        var item = Game_Enemy.prototype.itemObject(di.kind, di.dataId);
        var successRate = Math.round(
          (1 / this._beast.dropItems[j].denominator) * 100,
        );
        ...
        this.drawItemName(
          item,
          this.textWidth("100%") + stv_BeastBook_padding,
          y,
          maxLength,
        );
      } else {
  ...
  if (
    !this._beast.dropItems[0].kind &&
    !this._beast.dropItems[1].kind &&
    !this._beast.dropItems[2].kind
  ) {
    this.changeTextColor(this.textColor(stv_BeastBook_unknownColor));
...
Window_BeastBook_Info.prototype.contentDrawAbilities = function () {
  ...
  if (this._beast.actions.length <= 0) {
    this.changeTextColor(this.textColor(stv_BeastBook_unknownColor));
    this.drawText(
      stv_BeastBook_noData,
      this.contents.width - this.textWidth(stv_BeastBook_noData),
      y,
    );
    this.changeTextColor(this.normalColor());
  }
...
Window_BeastBook_Info.prototype.contentDrawExp = function () {
  ...
  this.contents.fillRect(
    0,
    this.contents.height - y * 2 - stv_BeastBook_padding,
    this.contents.width,
    y,
    this.gaugeBackColor(),
  );
  ...
  this.drawIcon(
    stv_BeastBook_expIcon,
    this.contents.width - 32,
    this.contents.height - y * 2 + 2 - stv_BeastBook_padding,
  );
  this.drawText(
    TextManager.exp + ":",
    0,
  ...
  this.drawText(
    ...
    this.contents.width -
      32 -
      this.textWidth(this._beast.exp) -
      stv_BeastBook_padding,
    this.contents.height - y * 2 - stv_BeastBook_padding,
  );
};
...
Window_BeastBook_Info.prototype.contentDrawGold = function () {
  ...
  this.drawIcon(
    stv_BeastBook_goldIcon,
    this.contents.width - 32,
    this.contents.height - y + 2,
  );
  ...
  this.drawText(
    ...
    this.contents.width -
      32 -
      this.textWidth(this._beast.gold) -
      stv_BeastBook_padding,
    this.contents.height - y,
  );
};
...
Window_BeastBook_Info.prototype.contentDrawBeast = function () {
  ...
  if (beast.meta.BeastBookPic) {
    this._bitmap = ImageManager.loadNormalBitmap(
      beast.meta.BeastBookPic + ".png",
      0 || 0,
    );
    ...
    if (note.match(/<(?:SIDEVIEW BATTLER):[ ](.*)>/i)) {
      ...
      if ($gameSystem.isSideView()) {
        this._bitmap = ImageManager.loadSvEnemy(
          $dataEnemies[beast.id].battlerName,
          beast.battlerHue,
        );
      } else {
        this._bitmap = ImageManager.loadEnemy(
          $dataEnemies[beast.id].battlerName,
          beast.battlerHue,
        );
      }
...
Window_BeastBook_Info.prototype.drawBeastASV = function () {
  ...
  if (bitmap) {
    ...
    var cx =
      Math.floor(stv_BeastBook_asvPattern / 6) * 3 +
      this.aSVMotion(stv_BeastBook_asvSpeed, 2);
    var cy = stv_BeastBook_asvPattern % 6;
    this._beastSprite.setFrame(cx * bw, cy * bh, bw, bh);
    this._beastBackSprite.setFrame(0, 0, bw, bh);
    if (stv_BeastBook_showBgBeast == "TRUE")
      this._beastBackSprite.bitmap = bitmap;
  }
};
...
Window_BeastBook_Info.prototype.contentDrawKillCounter = function () {
  ...
  this.drawGauge(
    gaugeXpos,
    gaugeYpos - stv_BeastBook_padding,
    gaugeWidth,
    actualkillsCount / maxKillsCount,
    this.textColor(stv_BeastBook_killsCountColor1),
    this.textColor(stv_BeastBook_killsCountColor2),
  );
  this.drawText(
    stv_BeastBook_killsText,
    gaugeXpos - this.textWidth(stv_BeastBook_killsText) - stv_BeastBook_padding,
    gaugeYpos,
  );
  this.drawText(
    killsCountText,
    this.contents.width - this.textWidth(killsCountText),
    gaugeYpos,
  );
};
...
Window_BeastBook_Info.prototype.refresh = function () {
  ...
  if ($beastBook.isRevealed(this._beast.id)) {
    ...
    if (
      $gameSwitches.value(stv_BeastBook_showItemsSwitch) ||
      !stv_BeastBook_showItemsSwitch
    )
      this.contentDrawItems();
    if (
      $gameSwitches.value(stv_BeastBook_showSkillsSwitch) ||
      !stv_BeastBook_showSkillsSwitch
    )
      this.contentDrawAbilities();
    if (
      $gameSwitches.value(stv_BeastBook_showKillCounterSwitch) ||
      !stv_BeastBook_showKillCounterSwitch
    )
      this.contentDrawKillCounter();
    if (
      $gameSwitches.value(stv_BeastBook_showExpSwitch) ||
      !stv_BeastBook_showExpSwitch
    )
      this.contentDrawExp();
    if (
      $gameSwitches.value(stv_BeastBook_showGoldSwitch) ||
      !stv_BeastBook_showGoldSwitch
    )
      this.contentDrawGold();
  }
};
...
Window_BeastBook_Elements.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_BeastBook_Elements.prototype.contentDrawResistance = function () {
  ...
  for (var i = 0; i < 30; i++) {
    if (i < this._beast.traits.length) {
      if (this._beast.traits[i].code === 11) {
        ...
        this.drawTextEx(
          $dataSystem.elements[this._beast.traits[i].dataId],
          0,
          y * ii,
        );
        ...
        this.drawText(
          Math.round(this._beast.traits[i].value * 100) + "%",
          Graphics.boxWidth / 3 - 110,
          y * ii,
          70,
          "right",
        );
  ...
  if (counter <= 0) {
    ...
    this.drawText(
      stv_BeastBook_noData,
      this.contents.width / 2 - this.textWidth(stv_BeastBook_noData) / 2,
      y,
    );
    this.changeTextColor(this.normalColor());
  }
...
Window_BeastBook_Elements.prototype.refresh = function () {
  ...
  this.drawText(
    stv_BeastBook_weaknessText,
    this.contents.width / 2 - this.textWidth(stv_BeastBook_weaknessText) / 2,
    0,
  );
...
Window_BeastBook_Parameters.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_BeastBook_Parameters.prototype.contentDrawLeftParameters = function () {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    this.resetTextColor();
    this.drawText(
      this._beast.params[i],
      leftEdgePos -
        stv_BeastBook_padding -
        this.textWidth(this._beast.params[i]),
      y,
      60,
    );
...
Window_BeastBook_Parameters.prototype.contentDrawRightParameters = function () {
  ...
  for (var j = 4; j < 8; j++) {
    ...
    this.drawText(
      TextManager.param(j),
      rightEdgePos + stv_BeastBook_padding,
      y,
      160,
    );
    this.resetTextColor();
    this.drawText(
      this._beast.params[j],
      this.contents.width -
        stv_BeastBook_padding -
        this.textWidth(this._beast.params[j]),
      y,
      60,
    );
    y += this.lineHeight() + stv_BeastBook_padding;
...
Beast_Book.prototype.clear = function () {
  ...
  if (stv_BeastBook_maxBeasts && stv_BeastBook_maxBeasts < maxBeasts)
    maxBeasts = stv_BeastBook_maxBeasts + 1;
  ...
  for (var i = 1; i < maxBeasts; ++i) {
    ...
    if (!this.beasts[i].discoveredItems)
      this.beasts[i].discoveredItems = [false, false, false];
    if (!this.beasts[i].kills) this.beasts[i].kills = 0;
    if (!this.beasts[i]._battleBack1)
      this.beasts[i]._battleBack1 = stv_BeastBook_defaultBattleBack;
    if (!this.beasts[i]._battleBack2)
      this.beasts[i]._battleBack2 = stv_BeastBook_defaultBattleBack;
...
Beast_Book.prototype.killAchievmentCheck = function (beastID) {
  ...
  if (beast.kills == beast.maxKills) {
    ...
    if (stv_BeastBook_killAchievmentSwitch)
      $gameSwitches.setValue(achievmentSwitch, true);
  }
};
...
Beast_Book.prototype.getRevealed = function () {
  ...
  if (discoveredLength >= $dataEnemies.length - 1)
    $gameSwitches.setValue(stv_BeastBook_bookFullSwitch, true);
};
...
Beast_Book.prototype.discoverItem = function (E_Id, item) {
  ...
  for (var i = 0; i < 3; i++) {
    if (
      $dataEnemies[E_Id].dropItems[i].kind == itemKind &&
      $dataEnemies[E_Id].dropItems[i].dataId == item.id
    ) {
...
Window_Base.prototype.drawSkillName = function (skill, x, y, width) {
  if (skill) {
    if (this.textWidth(skill.name) > width) {
      this.drawText(
        skill.name,
        x + this.textWidth(skill.name) - width,
        y,
        width,
      );
    } else {
