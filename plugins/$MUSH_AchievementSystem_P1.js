...
MUSH_Achievements.prototype.createSceneDetails = function () {
  ...
  //   - One line Max.
  //-----------------------------------------------
  this._snDetails.sceneDescription =
    "Press" +
    "\\c[" +
    31 +
    "]" +
    ...
    0 +
    "]" +
    "button to toggle the scene view!";
  //-----------------------------------------------
  ...
  //-----------------------------------------------
  this._snDetails.iconNotUnlocked = 2930;
  ...
  //   - For alpha, the value must be between 0 and 1 (decimal is ok)
  //-----------------------------------------------
  this._snDetails.completionColor1 = "rgba(0, 195, 213,1)";
  this._snDetails.completionColor2 = "rgba(0, 98, 178, 1)";
  //-----------------------------------------------
  // * Logo Size
...
MUSH_Achievements.prototype.createAchievementsList = function () {
  var achList = [
    ...
    //          "4/0/300" = Receive 300 EXP.
    //          "5/0/2" = Receive 2 Gems.
    //          "-1/0/0"   = No reward
    //
    ...
    [
      ...
      1436,
      "", //  ! image file
      "Open the menu screen for the first time",
      "3/0/100",
      "0/28/0",
      false,
      true,
    ...
    [
      ...
      692,
      "", //  ! image file
      "Buy a Golden Comb in the Armor Shop at the Delta Village",
      "5/0/3",
      "0/52/0",
      false,
      false,
    ...
    [
      ...
      735,
      "", //  ! image file
      "Complete the given first quest",
      "4/0/100",
      "1/19/1",
      false,
      false,
    ...
    [
      ...
      6851,
      "", //  ! image file
      "Kill 25 Enemies with" +
        "\\i[" +
        1088 +
        "]" +
        "Physical Elemental Skills or Items",
      "3/0/500",
      "2/66/25",
      false,
      false,
    ...
    [
      ...
      6825,
      "", //  ! image file
      "Kill 20 Enemies with" +
        "\\i[" +
        1330 +
        "]" +
        "Fire Elemental Skills or Items",
      "4/0/150",
      "2/67/20",
      false,
      false,
...
Scene_mushMenuAchievementP1.prototype.createWindowGeneral = function () {
  this._windowTitle = new Window_mushMenuAchievementTitle_P1(
    0,
    0,
    Graphics.width,
    144,
  ...
  this._windowCommand = new Window_mushMenuAchievementCommand_P1(
    0,
    this._windowTitle.height,
    Graphics.width,
    Graphics.height - this._windowTitle.height,
    this._ach
  );
  ...
  this._windowCommand2 = new Window_mushMenuAchievementCommand2_P1(
    0,
    this._windowTitle.height,
    Graphics.width,
    wc2h,
  ...
  this._windowHelpCm2 = new Window_mushMenuAchievementCommand_P1(
    0,
    this._windowTitle.height + this._windowCommand2.height,
    Graphics.width,
    whc2h,
    this._ach,
    true
...
Scene_Map.prototype.update = function () {
  ...
  if (this._wPopPeriod > 0) {
    if (
      Graphics.frameCount % this._wPopPeriod == 0 &&
      this.achievementSwitchCondition()
    ) {
      this.verifyAchievementCompletion();
    }
  }
...
var aliasMush_SceneMenuCreateCommandWindow28 =
...
Scene_Menu.prototype.createCommandWindow = function () {
  ...
  if (dtAch.addToMenu) {
    ...
    this._commandWindow.setHandler(
      symbol,
      this.commandMushAchievements.bind(this)
    );
  }
};
...
Window_mushMenuAchievementTitle_P1.prototype = Object.create(
  Window_Base.prototype
);
Window_mushMenuAchievementTitle_P1.prototype.constructor =
  Window_mushMenuAchievementTitle_P1;

Window_mushMenuAchievementTitle_P1.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_mushMenuAchievementTitle_P1.prototype.drawCompletion = function (
  barWidth
) {
  var dt = this._ach.getSceneDetails();
...
Window_mushMenuAchievementTitle_P1.prototype.drawTextExCenter = function (
  text,
  x,
  y,
  width
...
Window_mushMenuAchievementCommand_P1.prototype = Object.create(
  Window_Selectable.prototype
);
Window_mushMenuAchievementCommand_P1.prototype.constructor =
  Window_mushMenuAchievementCommand_P1;

Window_mushMenuAchievementCommand_P1.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_mushMenuAchievementCommand_P1.prototype.drawItem = function (
  index,
  rect
) {
  ...
  if (it.imageFile != "" && it.imageFile != undefined && it.imageFile != null) {
    ...
    if (paAc.unlocked == false) {
      if (
        sd.spriteNotUnlocked != "" &&
        sd.spriteNotUnlocked != undefined &&
        sd.spriteNotUnlocked != null
      ) {
...
Window_mushMenuAchievementCommand_P1.prototype.changeColor1 = function (
  unlocked,
  sd
) {
  if (unlocked == true) {
...
Window_mushMenuAchievementCommand_P1.prototype.changeColor2 = function (
  unlocked,
  sd
) {
  if (unlocked == true) {
...
Window_mushMenuAchievementCommand_P1.prototype.drawIconPlusZoom = function (
  iconIndex,
  x,
  y,
  zoom
...
Window_mushMenuAchievementCommand_P1.prototype.getRewardName = function (
  it,
  sd
) {
  ...
  if (s.category < 3 && s.category >= 0) {
    ...
    var text = " " + " \\c[" + 17 + "]" + s.amount + " \\i[" + 858 + "]";
    return text;
  } else if (s.category == 4) {
    var text = " " + "\\c[" + 6 + "]" + s.amount + " \\i[" + 2073 + "]";
    return text;
  ...
  for (var i = 0; i < this._sprt.children.length; i++) {
    if (this._sprt.children[i].getRepos() == false) {
      if (this._sprt.children[i].width && this._sprt.children[i].height) {
        var scaleX =
          this._ach.getSceneDetails().logoSize / this._sprt.children[i].width;
        var scaleY =
          this._ach.getSceneDetails().logoSize / this._sprt.children[i].height;
...
Window_mushMenuAchievementCommand_P1.prototype.updateHelp = function (index) {
  if (this._hIndex != index) {
    ...
    if (
      this._hIndex >= 0 &&
      this._hIndex <= this._ach.getAchList().length - 1
    ) {
      this.contents.clear();
      for (var i = 0; i < this._sprt.children.length; i++) {
        this._sprt.children[i].renInactive();
...
Window_mushMenuAchievementPop_P1.prototype = Object.create(
  Window_Base.prototype
);
Window_mushMenuAchievementPop_P1.prototype.constructor =
  Window_mushMenuAchievementPop_P1;
...
Window_mushMenuAchievementPop_P1.prototype.refresh = function () {
  ...
  this.contents.gradientFillRect(
    ...
    0,
    wd,
    (this._achs.length / 2) * this.lineHeight(),
    color1,
    color2,
    true
  );
...
Window_mushMenuAchievementCommand2_P1.prototype = Object.create(
  Window_Selectable.prototype
);
Window_mushMenuAchievementCommand2_P1.prototype.constructor =
  Window_mushMenuAchievementCommand2_P1;

Window_mushMenuAchievementCommand2_P1.prototype.initialize = function (
  x,
  y,
  width,
  height,
...
Window_mushMenuAchievementCommand2_P1.prototype.maxCols = function () {
  var cols = Math.floor(
    (this.width - 36) / (this.itemWidth() + this.spacing())
  );
  return cols;
};
...
Window_mushMenuAchievementCommand2_P1.prototype.setWindowHelp = function (
  windowHelp
) {
...
Window_mushMenuAchievementCommand2_P1.prototype.drawIconPlusZoom = function (
  iconIndex,
  x,
  y,
  zoom
...
Window_mushMenuAchievementCommand2_P1.prototype.itemRectForText = function (
  index
) {
...
var aliasMush_WindowMenuCommandMakeCommandList59 =
...
Game_Party.prototype.giveAchievementReward = function (dt, index) {
  ...
  else if (rw.category == 4) {
    $gameParty.allMembers().forEach(function (actor) {
      actor.gainExp(rw.amount);
    });
  } //  GEM REWARD (CUSTOM)
