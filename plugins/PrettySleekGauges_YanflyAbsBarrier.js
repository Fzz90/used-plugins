...
if (Imported.PrettySleekGauges && Imported.YEP_AbsorptionBarrier) {
  (function () {
    ...
    var hpBarTypeLeft =
      parameters["HP Bar Shape"].length === 2
        ? String(parameters["HP Bar Shape"]).substring(0, 1)
        : false;
    var hpBarTypeRight =
      parameters["HP Bar Shape"].length === 2
        ? String(parameters["HP Bar Shape"]).substring(1, 2)
        : false;
    var mpBarTypeLeft =
      parameters["MP Bar Shape"].length === 2
        ? String(parameters["MP Bar Shape"]).substring(0, 1)
        : false;
    var mpBarTypeRight =
      parameters["MP Bar Shape"].length === 2
        ? String(parameters["MP Bar Shape"]).substring(1, 2)
        : false;
    var tpBarTypeLeft =
      parameters["TP Bar Shape"].length === 2
        ? String(parameters["TP Bar Shape"]).substring(0, 1)
        : false;
    var tpBarTypeRight =
      parameters["TP Bar Shape"].length === 2
        ? String(parameters["TP Bar Shape"]).substring(1, 2)
        : false;
    ...
    var drawEnemyMPWhenNoMP =
      (parameters["Show MP Bar When MMP is 0"] || "true") === "true";
    ...
    Special_Gauge.prototype.initialize = function (
      x,
      y,
      w,
      r,
      ...
      alias_special_gauge_initialize.call(
        this,
        x,
        y,
        w,
    ...
    Special_Gauge.prototype.doneUpdating = function () {
      return (
        alias_special_gauge_doneUpdating.call(this) &&
        (this._type === "hp"
    ...
    Special_Gauge.prototype.update = function () {
      ...
      if (this._type === "hp") {
        if (this._curAbspRate > this._maxAbspRate)
          this._curAbspRate -= this._abspSpdRate;
        ...
        if (!animatedNumbers || this.shouldntAnimate())
          this._curAbsp = this._maxAbsp;
        if (!animatedGauges || this.shouldntAnimate())
          this._curAbspRate = this._maxAbspRate;
      }
      ...
      if (typeof rate === "number") {
        ...
        if (rate.barrierPoints() > 0) {
          ...
          if (rate1 != this._maxRate) {
            this._maxRate = rate1;
            if (typeof this._curRate !== "number")
          ...
          if (rate2 != this._maxAbspRate) {
            ...
            if (this._curAbspRate === undefined)
              this._curAbspRate = this._maxAbspRate;
            this._abspSpdRate =
              Math.abs(this._curAbspRate - this._maxAbspRate) / 60;
          }
    ...
    Special_Gauge.prototype.drawGauge = function () {
      ...
      this._window.contents.drawTrap(
        this._x,
        gy,
        this._width - 2,
        this._height,
        this._window.gaugeBackColor(),
        true
      );
    ...
    Special_Gauge.prototype.drawText = function () {
      if (this._vocab) {
        ...
        if (this._showEHPText) {
          ...
          if (this._type === "hp" && this._curAbsp > 0) {
            var abspWidth = this._window.textWidth(
              " +" + Math.round(this._curAbsp)
            );

            if (
              this._maxVal &&
              abspWidth +
                this._window.textWidth(
                  (this._setVal || this._curVal) + " / " + this._maxVal
                ) <=
                width
            ) {
              this._window.drawText(
                Math.round(this._curVal),
                x,
                this._y + this._yOffset,
                width -
                  abspWidth -
                  this._window.textWidth(" / " + this._maxVal),
                "right"
              );
              var color = "#";
              for (var i = 0; i < 3; i++)
                color += Yanfly.Param.ABRPop[i].toString(16).padZero(2);
              ...
              this._window.textWidth(this._setVal || this._curVal) +
                abspWidth <=
              width
            ) {
              this._window.drawText(
              ...
              this._window.changeTextColor(color);
              this._window.drawText(
                Math.round(this._curVal + this._curAbsp),
                x,
                this._y + this._yOffset,
                width,
                "right"
