...
Game_Interpreter.prototype.setPluginCommandBossHP = function (command, args) {
  ...
  if (command == "boss_hp_erase") {
    $gameTemp._battler_bhp_remove = "true";
  }
  if (Imported.MOG_ChronoEngine) {
...
Scene_Battle.prototype.update = function () {
  ...
  if ($gameTemp._forceCreateBossHud) {
    this.recreateBossHP();
  }
};
...
Spriteset_Map.prototype.createLowerLayer = function () {
  ...
  if (Imported.MOG_ChronoEngine) {
    if ($gameTemp._forceCreateBossHud2) {
      this.forceCreateBossHud();
    }
    this.sortMz();
  }
};
...
Spriteset_Map.prototype.update = function () {
  ...
  if ($gameTemp._forceRemoveBossHud) {
    this.forceRemoveBossHud();
  }
};
...
Sprite_BossHP.prototype.createSprites = function () {
  ...
  this.createLayout();
  this.createHPMeter();
  if (String(Moghunter.bosshp_face) === "true") {
    this.createFace();
  }
  this.createHPNumber();
  if (String(Moghunter.bosshp_states) === "true") {
    this.createStates();
  }
  this.createName();
};
...
Sprite_BossHP.prototype.refreshStates = function () {
  ...
  this._stateIcons = [];
  if (this._battler.allIcons().length == 0 || this._battler.isDead()) {
    return;
  }
...
Sprite_BossHP.prototype.update = function () {
  ...
  if (this.need_onSkillCancelites()) {
    this.onSkillCancelites(5);
  } else {
    this.onSkillCancelites(-5);
  }
