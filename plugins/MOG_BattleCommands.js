...
Moghunter.bcom_zoom_rate = Number(Moghunter.parameters["Zoom Rate"] || 1.3);
...
if (Imported.MOG_BattleHud) {
  ...
  Window_ActorCommand.prototype.setup = function (actor) {
    ...
    if (this._actor) {
      this.battle_commands_initialize();
    }
  };
  ...
  Window_ActorCommand.prototype.clearComSprites = function () {
    ...
    if (this._cursor_b) {
      this.removeChild(this._cursor_b);
      this._cursor_b = null;
    }
    if (this._com_name) {
      this.removeChild(this._com_name);
      this._com_name = null;
    }
    if (this._arrow) {
      for (var i = 0; i < 2; i++) {
  ...
  Window_ActorCommand.prototype.com_pic_initialize = function () {
    ...
    if (String(Moghunter.bcom_side_input) === "true") {
      this._side_input = true;
    }
    if (String(Moghunter.bcom_ring_anime) === "true") {
      this._ring_move = true;
    }
    this._rol_range = Moghunter.bcom_ring_range;
    this._pi = Moghunter.bcom_pi_range * Math.PI;
    if (String(Moghunter.bcom_slide_effect) === "true") {
      this._side_m = [Moghunter.bcom_slide_x, Moghunter.bcom_slide_y];
    }
    this._row_max = Math.max(Moghunter.bcom_row_max - 1, 1);
    this._zoom_speed = Math.max(Moghunter.bcom_zoom_speed, 0.001);
    this._zoom_rate = Math.max(Moghunter.bcom_zoom_rate, 1.0);
    this._zoom_effect = false;
    if (String(Moghunter.bcom_zoom_effect) === "true") {
      this._zoom_effect = true;
    }
    this._loop_zoom_effect = false;
    if (String(Moghunter.bcom_zoom_loop) === "true") {
      this._loop_zoom_effect = true;
    }
  };
  ...
  Window_ActorCommand.prototype.load_com_images = function () {
    ...
    for (var i = 0; i < this._list.length; i++) {
      if (this._max_com < this._list.length) {
        this._max_com = this._list.length;
      }
    ...
    if (String(Moghunter.bcom_arrow) === "true") {
      this._arrow_img = ImageManager.loadBcom("Arrow");
    }
  };
  ...
  Window_ActorCommand.prototype.create_com_sprites = function () {
    this.create_layout();
    if (String(Moghunter.bcom_face) === "true") {
      this.create_face();
    }
    this.create_commands();
    if (String(Moghunter.bcom_cursor) === "true") {
      this.create_cursor();
    }
    if (String(Moghunter.bcom_arrow) === "true") {
      this.create_arrows();
    }
    if (String(Moghunter.bcom_com_name) === "true") {
      this.create_com_name();
    }
  };
  ...
  Window_ActorCommand.prototype.create_cursor = function () {
    ...
    if (String(Moghunter.bcom_cursor_slide) === "true") {
      this._cursor_slide = true;
    }
  ...
  Window_ActorCommand.prototype.update_cursor_b = function () {
    if (this._cursor_slide) {
      this.update_cursor_slide();
    }
  ...
  Window_ActorCommand.prototype.update_cursor_slide = function () {
    ...
    if (this._cursor_b_s[4] < 2) {
      return;
    }
    this._cursor_b_s[4] = 0;
    this._cursor_b_s[3] += 1;
    if (this._cursor_b_s[3] < 15) {
      this._cursor_b_s[2] += 1;
    } else if (this._cursor_b_s[3] < 30) {
      this._cursor_b_s[2] -= 1;
    } else {
      this._cursor_b_s[2] = 0;
      this._cursor_b_s[3] = 0;
    }
  };
  ...
  Window_ActorCommand.prototype.refresh_bitmap_com = function () {
    for (var i = 0; i < this.maxCom(); i++) {
      ...
      if (this._wh[1] < this._com_sprites[i].bitmap.height) {
        this._wh[1] = this._com_sprites[i].bitmap.height;
      }
    }
    for (var i = 0; i < this.maxCom(); i++) {
  ...
  Window_ActorCommand.prototype.position_clear = function () {
    ...
    for (var i = 0; i < this.maxCom(); i++) {
      ...
      this._com_sprites[i].scale.x = 1.0;
      this._com_sprites[i].scale.y > 1.0;
    }
    if (this._com_mode_b === 1) {
      this.refresh_ring_command();
    }
  ...
  Window_ActorCommand.prototype.needResetCom = function () {
    ...
    if (this._actor && this._oldID != this._actor._actorId) {
      return true;
    }
    return false;
  };
  ...
  Window_ActorCommand.prototype.update_battle_commands = function () {
    ...
    this.opacity = 0;
    if (this._com_index != this._index) {
      this.refresh_index();
    }
    for (var i = 0; i < this._com_sprites.length; i++) {
      if (i <= this.maxCom()) {
    ...
    if (this._old_visible != this.visible) {
      this.position_clear();
    }
    if (this._arrow) {
      this.update_arrow();
    ...
    if (this._cursor_b) {
      this.update_cursor_b();
    }
    if (this._com_name) {
      this._com_name.opacity += this.com_fade_speed();
    }
  ...
  Window_ActorCommand.prototype.update_commands = function (i) {
    ...
    if (!this._com_sprites[i].enabled) {
      this._com_sprites[i].opacity = 90;
    }
  };
  ...
  Window_ActorCommand.prototype.update_on_index = function (i) {
    ...
    if (this._zoom_effect) {
      this.update_zoom_effect(i);
    }
  };
  ...
  Window_ActorCommand.prototype.update_off_index = function (i) {
    this._side[i] = [0, 0];
    if (this._com_sprites[i].opacity > this.translucentOpacity()) {
      this._com_sprites[i].opacity -= this.com_fade_speed();
    }
    if (this._com_mode_b === 0) {
    ...
    if (this._com_sprites[i].scale.x > 1.0) {
  ...
  Window_ActorCommand.prototype.update_zoom_effect = function (i) {
    if (this._zoom[0] === 0) {
      if (this._com_sprites[i].scale.x < this.com_zoom_rate()) {
        this._com_sprites[i].scale.x += this.com_zoom_speed();
      }
      if (this._loop_zoom_effect && this._com_sprites[i].scale.x >= this.com_zoom_rate()) {
        this._zoom[0] = 1;
      }
    } else {
      this._com_sprites[i].scale.x -= this.com_zoom_speed();
  ...
  Window_ActorCommand.prototype.isTouchedInsideCom = function () {
    for (var i = 0; i < this._com_sprites.length; i++) {
      if (this._com_sprites[i].visible && this._com_sprites[i].opacity > 25) {
        if (
          TouchInput.x > this.x + this._com_sprites[i].x - this._com_sprites[i].bitmap.width / 2 &&
  ...
  Window_ActorCommand.prototype.update = function () {
    ...
    if ($gameTemp._bcom_need_refresh) {
      $gameTemp._bcom_need_refresh = false;
      this.battle_commands_initialize();
    }
    if (this.sel_time > 0) {
      this.update_wait_selection();
      return;
    }
  ...
  Scene_Battle.prototype.create_layout_window = function () {
    ...
    if (this._com_layout) {
      this.removeChild(this._com_layout);
    }
  };
}
