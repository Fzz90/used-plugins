...
Yanfly.SkillMastery.version = 1.0;
...
DataManager.processSkillMasteryLevelsNotetags1 = function (group) {
  ...
  for (var n = 1; n < group.length; n++) {
    ...
    obj.masteryFormula = new Function("level", "skill", "return " + Yanfly.Param.SMLLvFormula);
    obj.masteryDamage = new Function("value", "level", "skill", "return " + Yanfly.Param.SMLDmgFormula);
    obj.masteryMpCost = new Function("cost", "level", "skill", "return " + Yanfly.Param.SMLmpFormula);
    obj.masteryTpCost = new Function("cost", "level", "skill", "return " + Yanfly.Param.SMLtpFormula);
    obj.masteryHpCost = new Function("cost", "level", "skill", "return " + Yanfly.Param.SMLhpFormula);
    obj.masteryCooldown = new Function("turns", "level", "skill", "return " + Yanfly.Param.SMLcdFormula);
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(/<(?:MAX MASTERY LEVEL):[ ](\d+)>/i)) {
        ...
        if (type === "DAMAGE") {
          obj.masteryDamage = new Function("value", "level", "skill", "return value + (level * " + value + ")");
        } else if (type === "MP COST") {
          obj.masteryMpCost = new Function("cost", "level", "skill", "return cost + (level * " + value + ")");
        } else if (type === "TP COST") {
          obj.masteryTpCost = new Function("cost", "level", "skill", "return cost + (level * " + value + ")");
        } else if (type === "HP COST") {
          obj.masteryHpCost = new Function("cost", "level", "skill", "return cost + (level * " + value + ")");
        } else if (type === "COOLDOWN") {
          obj.masteryCooldown = new Function("turns", "level", "skill", "return turns + (level * " + value + ")");
        }
        ...
        if (type === "DAMAGE") {
          obj.masteryDamage = new Function("value", "level", "skill", "return value * (1.00 + (level * " + value + "))");
        }
        if (type === "MP COST") {
          obj.masteryMpCost = new Function("cost", "level", "skill", "return cost * (1.00 + (level * " + value + "))");
        } else if (type === "TP COST") {
          obj.masteryTpCost = new Function("cost", "level", "skill", "return cost * (1.00 + (level * " + value + "))");
        } else if (type === "HP COST") {
          obj.masteryHpCost = new Function("cost", "level", "skill", "return cost * (1.00 + (level * " + value + "))");
        } else if (type === "COOLDOWN") {
          obj.masteryCooldown = new Function("turns", "level", "skill", "return turns * (1.00 + (level * " + value + "))");
        }
        // No Mastery Change
      } else if (line.match(/<NO[ ](.*)[ ]MASTERY EFFECT>/i)) {
        var type = String(RegExp.$1).toUpperCase();
        if (type === "DAMAGE") {
          obj.masteryDamage = new Function("value", "level", "skill", "return value");
        } else if (type === "MP COST") {
          obj.masteryMpCost = new Function("cost", "level", "skill", "return cost");
        } else if (type === "TP COST") {
          obj.masteryTpCost = new Function("cost", "level", "skill", "return cost");
        } else if (type === "HP COST") {
          obj.masteryHpCost = new Function("cost", "level", "skill", "return cost");
        } else if (type === "COOLDOWN") {
          obj.masteryCooldown = new Function("turns", "level", "skill", "return turns");
        }
        ...
        if (type === "EXP") {
          obj.masteryFormula = new Function("level", "skill", "return " + formula);
        } else if (type === "DAMAGE") {
          obj.masteryDamage = new Function("value", "level", "skill", "return " + formula);
        } else if (type === "MP COST") {
          obj.masteryMpCost = new Function("cost", "level", "skill", "return " + formula);
        } else if (type === "TP COST") {
          obj.masteryTpCost = new Function("cost", "level", "skill", "return " + formula);
        } else if (type === "HP COST") {
          obj.masteryHpCost = new Function("cost", "level", "skill", "return " + formula);
        } else if (type === "COOLDOWN") {
          obj.masteryCooldown = new Function("turns", "level", "skill", "return " + formula);
        }
        // End
      ...
      if (line.match(/<STARTING SKILL (?:MASTERY|MASTERIES)>/i)) {
        ...
        if (line.match(/(\d+),[ ](\d+)/i)) {
          ...
          try {
            ...
            Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery EXP Formula for " + $dataSkills[skillId].name, e);
          }
...
Yanfly.SkillMastery.Game_BattlerBase_initMembers = Game_BattlerBase.prototype.initMembers;
...
Game_BattlerBase.prototype.updateSkillMasteryUsageMax = function (skillId) {
  ...
  try {
    this._skillMasteryUsageMax[skillId] = skillFormula.call(this, this._skillMasteryLevels[skillId] + 1, $dataSkills[skillId]);
  } catch (e) {
    Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery EXP Formula for " + $dataSkills[skillId].name, e);
  }
};
...
Game_BattlerBase.prototype.gainSkillMasteryLevel = function (skillId, value) {
  ...
  this._skillMasteryLevels[skillId] = this._skillMasteryLevels[skillId].clamp(0, $dataSkills[skillId].masteryMaxLevel);
  this._skillMasteryUses[skillId] = 0;
  this.updateSkillMasteryUsageMax(skillId);
};
...
Game_BattlerBase.prototype.isMaxedSkillMasteryLevel = function (skill) {
  ...
  this.checkSkillMasteryLevels();
  return this.skillMasteryLevel(skillId) >= $dataSkills[skillId].masteryMaxLevel;
};
...
Yanfly.SkillMastery.Game_BattlerBase_paySkillCost = Game_BattlerBase.prototype.paySkillCost;
...
Yanfly.SkillMastery.Game_Battler_onAllActionsEnd = Game_Battler.prototype.onAllActionsEnd;
...
Yanfly.SkillMastery.Game_BattlerBase_skillMpCost = Game_BattlerBase.prototype.skillMpCost;
Game_BattlerBase.prototype.skillMpCost = function (skill) {
  ...
  if (cost > 0) {
    ...
    try {
      ...
      Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery MP Cost Formula for " + $dataSkills[skill.id].name, e);
    }
...
Yanfly.SkillMastery.Game_BattlerBase_skillTpCost = Game_BattlerBase.prototype.skillTpCost;
Game_BattlerBase.prototype.skillTpCost = function (skill) {
  ...
  if (cost > 0) {
    ...
    try {
      ...
      Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery TP Cost Formula for " + $dataSkills[skill.id].name, e);
    }
...
if (Imported.YEP_SkillCore) {
  Yanfly.SkillMastery.Game_BattlerBase_skillHpCost = Game_BattlerBase.prototype.skillHpCost;
  Game_BattlerBase.prototype.skillHpCost = function (skill) {
    ...
    if (cost > 0) {
      ...
      try {
        ...
        Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery HP Cost Formula for " + $dataSkills[skill.id].name, e);
      }
  ...
  if (Imported.YEP_X_SkillCooldowns) {
    Yanfly.SkillMastery.Game_BattlerBase_applyCooldownMods = Game_BattlerBase.prototype.applyCooldownMods;
    Game_BattlerBase.prototype.applyCooldownMods = function (skill) {
      ...
      if (turns > 0) {
        ...
        try {
          ...
          Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery Cooldown Formula for " + $dataSkills[skill.id].name, e);
        }
...
Yanfly.SkillMastery.Game_Action_makeDamageValue = Game_Action.prototype.makeDamageValue;
Game_Action.prototype.makeDamageValue = function (target, critical) {
  var value = Yanfly.SkillMastery.Game_Action_makeDamageValue.call(this, target, critical);
  if (value !== 0 && this.isSkill()) {
    var level = this.subject().skillMasteryLevel(this.item());
    try {
      value = Math.round(this.item().masteryDamage.call(this, value, level, this.item()));
    } catch (e) {
      Yanfly.Util.SkillMasteryLevelsError("SKILL MASTERY LEVELS:\n" + "Bad code for Mastery Damage Formula for " + this.item().name, e);
    }
  }
  return value;
};
...
Yanfly.SkillMastery.Window_Base_drawItemName = Window_Base.prototype.drawItemName;
