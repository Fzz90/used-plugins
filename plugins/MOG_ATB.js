...
Moghunter.atb_TurnDuration = Number(
  Moghunter.parameters["Turn Duration"] || 480,
);
Moghunter.atb_StatesDuration = Number(
  Moghunter.parameters["States Duration"] || 180,
);
Moghunter.atb_SkipEmerge = String(
  Moghunter.parameters["Skip Emerge Message"] || "true",
);
Moghunter.atb_EscapeDuration = Number(
  Moghunter.parameters["Escape Duration"] || 180,
);

Moghunter.atb_EscapeGaugeX = Number(
  Moghunter.parameters["Escape Layout X"] || 600,
);
Moghunter.atb_EscapeGaugeY = Number(
  Moghunter.parameters["Escape Layout Y"] || 64,
);
Moghunter.atb_EscapeGaugeX2 = Number(
  Moghunter.parameters["Escape Gauge X"] || 6,
);
Moghunter.atb_EscapeGaugeY2 = Number(
  Moghunter.parameters["Escape Gauge Y"] || 16,
);
Moghunter.atb_EscapeButton = String(
  Moghunter.parameters["Escape Button"] || "cancel",
);
Moghunter.atb_FullSE = String(Moghunter.parameters["Full SE"] || "Book1");
Moghunter.atb_NextActorLeft = String(
  Moghunter.parameters["Change Actor Left"] || "pagedown",
);
Moghunter.atb_NextActorRight = String(
  Moghunter.parameters["Change Actor Right"] || "pageup",
);
...
Game_BattlerBase.prototype.needForceClearSelection = function () {
  if (!BattleManager._atbBattlerInput) {
    return false;
  }
  if (BattleManager._atbBattlerInput[0] != this) {
    return false;
  }
...
Game_BattlerBase.prototype.canMoveAtb = function () {
  ...
  if (Imported.MOG_BattleCameraFrontal && $gameSystem._atbMode[0] < 3) {
    if ($gameTemp._bcam.atbwaitFocus && $gameTemp._bcam.sMov) {
      return false;
    }
  }
  return true;
...
// * Can Fill ATB
//==============================
Game_BattlerBase.prototype.canFillATB = function (battler) {
  if (this._wait_atb > 0) {
    return false;
  }
  return true;
...
// * Reset States Counts
//==============================
var _mog_atb_gbatbase_resetStateCounts =
...
//==============================
//CHANGE?
Game_BattlerBase.prototype.updateStateTurns_ATB = function (battler) {
  Scene_Battle.prototype.canUpdateAtbMode = function () {
    return true;
  };

  // if (battler.isStateCategoryAffected("freeze")) {
  //   turn -= 1;
  // }
...
Game_Battler.prototype.performActionEnd = function (stateId) {
...
BattleManager.needUpdate_ATBBasic = function () {
  ...
  if ($gameTemp._atbWaitTemp[0] > 0) {
    return false;
  }
  if ($gameTemp._atbWaitTemp[1] > 0) {
    return false;
  }
  if ($gameSystem._atbMode[1]) {
    return false;
  }
  return true;
};
...
BattleManager.needUpdate_ATB = function () {
  ...
  if ($gameTemp._atbInTurn) {
    return false;
  }
  return true;
};

// Game_BattlerBase.prototype.killCount = function() {
//   return this._battlerKillCount
// }
...
BattleManager.update_ATB = function () {
  ...
  if (this.actor()) {
    this.updateCheckActorPar(this.actor());
  }
  this.allBattleMembers().forEach(function (battler) {
    if (battler.canMoveAtb() && !battler.isDead()) {
      this.updateBattlers_ATB(battler);
    }
  }, this);
...
BattleManager.updateBattlers_ATB = function (battler) {
  if (battler._wait_atb > 0) {
    battler._wait_atb--;
  }
  if (battler.canFillATB()) {
    this.updateFill_ATB(battler);
  }
...
BattleManager.updateFill_ATB = function (battler) {
  if (battler.isEnemy()) {
    this._max_atb = Math.max(Moghunter.atb_MaxValue - 12, 100);
  }
  if (!battler.isMaxAtb()) {
    battler.updateCountAtb();
    this.updateParCorretionBattler_ATB(battler);
    if (battler.isMaxAtb()) {
      this.executeATBFullEffect(battler);
    }
  } else {
...
BattleManager.executeATBFullEffect = function (battler) {
  if (battler._atb > battler._max_atb) {
    battler._atb = battler._max_atb;
  }
  if (battler.isActor()) {
    SoundManager.playSoundMX2(Moghunter.atb_FullSE);
  }
...
BattleManager.updateATBCast_ATB = function (battler) {
  ...
  battler._cast_atb[1]++;
  if (battler._cast_atb[1] >= battler._cast_atb[2]) {
    this.prepareNextSubjectCast_ATB(battler);
  }
};
...
BattleManager.canEscape_ATB = function (active) {
  ...
  if (!Input.isPressed(Moghunter.atb_EscapeButton)) {
    return false;
  }
  return true;
};
...
// ============================================================================
// * Helper: jalankan efek ATB yang biasanya terjadi setelah die()
// * TANPA membunuh battler.
// ============================================================================

Game_BattlerBase.prototype.runDieAtbCleanup = function () {
  if ($gameParty.inBattle()) {
    if (this.needForceClearSelection && this.needForceClearSelection()) {
      if (this.refreshCommandSelectionATB) this.refreshCommandSelectionATB();
    }
    $gameTemp._atbneedRefWind = true;
    if (this.clearATB) this.clearATB();
  }
};
...
BattleManager.prepareNextSubject_ATB = function (battler) {
  console.log("test");
  // if ($gameActors.actor(1).isDying() || $gameActors.actor(2).isDying() ||$gameActors.actor(3).isDying() || $gameActors.actor(4).isDying()) {
  //   $gameTemp.reserveCommonEvent(207); // Misalnya: Aktifkan Beacon of Hope
  // } else {
  //   $gameTemp.reserveCommonEvent(208);
  // }

  // ! 2026 Update
  // if (battler.isStateAffected(1) && battler.isActor()) {
  // battler.clearATB();
  //   battler.runDieAtbCleanup();
  // }
  Scene_Battle.prototype.canUpdateAtbMode = function () {
    return false;
  };
  ...
  console.log(battler.isFreezing());

  if (
    battler.isStateCategoryAffected("paralyze") ||
    battler.isStateCategoryAffected("disable")
  ) {
    Scene_Battle.prototype.canUpdateAtbMode = function () {
      return false;
    };
    var subject = battler;
    // subject._stateTurns[38];
    // subject._stateTurns[41];
    subject.forceAction(96, -1);
  ...
  if (battler.isStateAffected(31) && !battler.isStateAffected(1)) {
    Scene_Battle.prototype.canUpdateAtbMode = function () {
      return false;
    };
    if (
      !battler.isEnemy() ||
      (battler.isEnemy() && !battler.isStateCategoryAffected("paralyze"))
    ) {
      //WORKING CODE!
      var sealedSkills = 3;
      console.log("skill sealed!");
      Game_BattlerBase.prototype.isSkillSealed = function (skillId) {
        return skillId >= sealedSkills && skillId !== 11;
      };
    }
  } else if (!battler.isStateAffected(31)) {
    if (
      !battler.isEnemy() ||
      (battler.isEnemy() && !battler.isStateCategoryAffected("paralyze"))
    ) {
  ...
  if (battler.isEnemy()) {
    ...
    if (battler.isConfused() || battler.isStateCategoryAffected("conf")) {
...
BattleManager.needPrepareActionActor = function (action) {
  ...
  if (!this._atbBattlerInput) {
    return false;
  }
  if (!this._atbBattlerInput[0]) {
    return false;
  }
  if (!this._atbBattlerInput[0].currentAction()) {
    return false;
  }
  if (!this._atbBattlerInput[0].currentAction().item()) {
    return false;
  }
  return true;
};
...
BattleManager.canUseItemATB = function () {
  if (!this._atbBattler._atbItem) {
    return false;
  }
  if (!this._atbBattler.canUse(this._atbBattler._atbItem)) {
    return false;
  }
  return true;
};
...
BattleManager.displayStartMessages = function () {
  if (String(Moghunter.atb_SkipEmerge) == "true") {
    return;
  }
...
Scene_Battle.prototype.updateBattleProcess = function () {
  if (Imported.MOG_EmergeMotion && $gameTemp.needSkipBattleProcessEM()) {
    ...
    if ($gameTemp._emergingInt > 0) {
      $gameTemp._emergingInt--;
    }
    return;
  }
  this.updateCommandsATB();
  if (!BattleManager.isAborting() || BattleManager.isBattleEnd()) {
    if (BattleManager.canUpdateEventPhase()) {
      BattleManager.updateEventPhase();
    }
    if (BattleManager.needUpdate_ATBBasic()) {
      if (BattleManager.canUpdateEventTurnATB()) {
        BattleManager.updateEventTurnATB();
      }
  ...
  if (Imported.MOG_BalloonActionName) {
...
Scene_Battle.prototype.updateAtbScBat = function () {
  if ($gameTemp._atbForceHideWindows) {
    this.forceHideWindows();
  }
  if ($gameTemp._atbWinData[0]) {
    this.forceHideWindowsTemp();
  }
  if ($gameTemp._atbWinData[1]) {
    this.loadWindowAtbData();
  }
  if ($gameTemp._atbneedRefWind) {
    this.forceRefreshWindowATB();
  }
  if ($gameTemp._atbWaitTemp[0] > 0) {
    $gameTemp._atbWaitTemp[0]--;
  }
  if ($gameTemp._atbWaitTemp[1] > 0) {
    $gameTemp._atbWaitTemp[1]--;
  }
...
Scene_Battle.prototype.canUpdateAtbMode = function () {
  ...
  } else if ($gameSystem._atbMode[0] === 3) {
...
//==============================
var _mog_atb_sBat_createDisplayObjects =
...
Scene_Battle.prototype.canStartCommandSelection = function () {
  if (!BattleManager._atbBattlerInput[0]) {
    return false;
  }
  if (BattleManager._atbBattlerInput[1]) {
    return false;
  }
  return true;
};
...
//==============================
var _mog_atb_createActorCommandWindow =
...
Sprite_Actor.prototype.isMoving = function () {
  if (this._actor && !this._actor._intTurn) {
    return false;
  }
...
// ** create Spriteset
//==============================
var _mog_atbEscapeGauge_sbattle_createSpriteset =
...
(() => {
  const _GB_gainHp = Game_BattlerBase.prototype.gainHp;
  Game_BattlerBase.prototype.gainHp = function (value) {
    _GB_gainHp.call(this, value);

    // Kalau di battle dan battler ini jadi dead/HP 0 setelah perubahan HP
    if ($gameParty.inBattle() && this.isDead && this.isDead()) {
      // 1) bersihin selection ATB bila ada (fungsi MOG)
      if (this.needForceClearSelection && this.needForceClearSelection()) {
        if (this.refreshCommandSelectionATB) this.refreshCommandSelectionATB();
      }

      // 2) flag refresh window ATB (punyamu)
      $gameTemp._atbneedRefWind = true;

      // 3) clear ATB battler yg mati (punya MOG)
      if (this.clearATB) this.clearATB();

      // 4) safety: kalau battler ini kebetulan sedang dipegang BattleManager sebagai subject/atbBattler, lepas
      if (BattleManager._subject === this) BattleManager._subject = null;
      if (BattleManager._atbBattler === this) BattleManager._atbBattler = null;

      // 5) safety: reset input state supaya battle loop lanjut
