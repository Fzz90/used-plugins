...
if (Imported.YEP_BuffsStatesCore) {
  ...
  Yanfly.StC.Game_BattlerBase_clearStates = Game_BattlerBase.prototype.clearStates;
  ...
  Game_Battler.prototype.isStateAddable = function (stateId) {
    ...
    if (state && state.category.contains("BYPASS DEATH REMOVAL")) {
      return !this.isStateResist(stateId) && !this._result.isStateRemoved(stateId) && !this.isStateRestrict(stateId);
    }
  ...
  Yanfly.StC.Game_Action_applyItemUserEffect = Game_Action.prototype.applyItemUserEffect;
...
(function () {
  var extractMetadataWithoutStateCategory = DataManager.extractMetadata;
  DataManager.extractMetadata = function (object) {
    extractMetadataWithoutStateCategory.call(DataManager, object);
    object.stateCategoryRates = {};
    var regex = /<state category rate:([^,]*),([^%>]*)(%?)>/gi;
    var match;
    while ((match = regex.exec(object.note))) {
      object.stateCategoryRates[match[1].trim().toUpperCase()] = parseFloat(match[2]) / (match[3] ? 100 : 1);
    }
  };

  var stateRateWithoutCategory = Game_BattlerBase.prototype.stateRate;
  Game_BattlerBase.prototype.stateRate = function (stateId) {
    var oldRate = stateRateWithoutCategory.call(this, stateId);
    var state = $dataStates[stateId];
    if (state) {
      var multiplyCategoryFunc = function (result, category) {
        var multiplyRateFunc = function (res, object) {
          var rate = object.stateCategoryRates[category];
          return rate === undefined ? res : res * rate;
        };
        return this.traitObjects().reduce(multiplyRateFunc, result);
      }.bind(this);
      return state.category.reduce(multiplyCategoryFunc, oldRate);
    } else {
      return oldRate;
    }
  };
